...
if ($stmt_verifica_respostas) {
    ...
    // Redireciona para a página do formulário se não houver respostas
...
// Atualiza as respostas do formulário no banco de dados
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $estacao = $_POST['estacao'] ?? '';
    $sexo = $_POST['sexo'] ?? '';
    $intensidade = $_POST['intensidade'] ?? '';
    $notas = implode(', ', $_POST['notas'] ?? []);
    $ocasiao = $_POST['ocasiao'] ?? '';
    $durabilidade = $_POST['durabilidade'] ?? '';

    $sql_update = "INSERT INTO respostas_formulario (cliente_email, estacao, sexo, intensidade, notas, ocasiao, durabilidade)
                   VALUES (?, ?, ?, ?, ?, ?, ?)
                   ...
                   notas = VALUES(notas),
                   ocasiao = VALUES(ocasiao),
                   durabilidade = VALUES(durabilidade)";

    $stmt_update = mysqli_prepare($conexao, $sql_update);

    if ($stmt_update) {
        mysqli_stmt_bind_param($stmt_update, "sssssss", $cliente_email, $estacao, $sexo, $intensidade, $notas, $ocasiao, $durabilidade);
